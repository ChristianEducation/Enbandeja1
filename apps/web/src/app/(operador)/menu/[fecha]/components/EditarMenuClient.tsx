"use client"
// ═══════════════════════════════════════════════════════════════════
// EditarMenuClient — Editar menú existente
// ═══════════════════════════════════════════════════════════════════
// Muestra opciones actuales, permite editar/agregar/eliminar
// Botones: Guardar cambios, Publicar, Cambiar estado
// ═══════════════════════════════════════════════════════════════════
...
interface EditarMenuClientProps {
  menuId: string
  fecha: string
  fechaDisplay: string
  estado: string
...
export function EditarMenuClient({
  menuId,
  fecha,
  fechaDisplay,
  estado,
  ...
  categorias,
  colegioNombre,
}: EditarMenuClientProps) {
  const router = useRouter()
  const [opciones, setOpciones] = useState<OpcionForm[]>(opcionesInit)
  ...
  const handleGuardar = async (publicar: boolean) => {
    ...
    try {
      ...
      const res = await fetch("/api/menu/actualizar", {
        method: "POST",
    ...
    <div className="p-5 md:p-8 space-y-6">
      ...
      <div>
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-display text-title font-bold text-foreground tracking-tight capitalize">
              {fechaDisplay}
            </h1>
            <p className="font-sans text-body text-foreground-secondary mt-0.5">
              {colegioNombre}
            </p>
          </div>
          <span
            className={`px-3 py-1 rounded-full font-display text-small font-semibold ${
              estado === "PUBLICADO"
      ...
      {!canEdit && (
        <div className="flex items-center gap-2 p-3 rounded-xl bg-warning/10 border border-warning/30">
          ...
          <p className="font-sans text-small text-warning">
            Este menú ya está {ESTADO_LABEL[estado]?.toLowerCase()}. No se puede editar.
          </p>
        </div>
      )}
      ...
      {/* Vista solo lectura si no se puede editar */}
      {!canEdit && (
        <div className="space-y-2">
          {opciones.map((op, index) => (
            <div
              key={op.id || index}
              className="rounded-xl bg-surface-glass border border-border p-4"
            >
